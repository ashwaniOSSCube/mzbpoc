...
<div class="filter">
    ...
    <ul class="filterList_" style="margin: 0px; list-style: none; padding: 0px;">
        ...
        foreach($values as $value):
            ...
            $label = sprintf($label, $this->translate(sprintf("general.filters.%s.%s", $this->label, $value['value'])));
            ?>
            <li>
                <div class="checkbox <?= $checkedClass ?> js_filterparent">
                    ...
                    <label class="checkbox-label js_optionfilter_option"><?= $label ?> (<?=$value['count'] ?>)</label>
                </div>
            </li>
        <?php endforeach; ?>
    </ul>
